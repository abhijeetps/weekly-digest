module.exports = (commits) => {
  ...
  if (commitsCount > 1) {
    ...
    for (i = 0; i < data.length; i++) {
      if (data[i].author.type !== 'Bot') {
        commitsString += `:hammer: [${data[i].commit.message}](${data[i].html_url}) by [${data[i].author.login}](${data[i].author.html_url})\n`
      }
    }
  } else if (commitsCount === 1) {
    commitsString += `This week, there have been ${commitsCount} commit in the repository. \nIt is:\n`
    commitsString += `:hammer: [${data[0].commit.message}](${data[0].html_url}) by [${data[0].author.login}](${data[0].author.html_url})\n`
  } else {
