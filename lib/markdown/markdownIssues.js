module.exports = (context, {issues}) => {
  ...
  for (i = 0; i < data.length; i++) {
    ...
    if (data[i].reactions.total_count > 0 && typeof likedData === 'undefined' && data[i].user.type !== 'Bot') {
      likedData = data[i]
    } else if (data[i].reactions.total_count > 0 && data[i].reactions.total_count > likedData.reactions.total_count && data[i].user.type !== 'Bot') {
      likedData = data[i]
    }
    if (data[i].comments > 0 && typeof noisyData === 'undefined' && data[i].user.type !== 'Bot') {
      noisyData = data[i]
    } else if (data[i].comments > 0 && data[i].comments > noisyData.comments && data[i].user.type !== 'Bot') {
      noisyData = data[i]
    }
  }
