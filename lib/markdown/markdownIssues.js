module.exports = (context, {issues}) => {
  ...
  for (i = 0; i < data.length; i++) {
    ...
    if (data[i].reactions.total_count > 0 && typeof likedData === 'undefined') {
      likedData = data[i]
    } else if (data[i].reactions.total_count > 0 && data[i].reactions.total_count > likedData.reactions.total_count) {
      likedData = data[i]
    }
    if (data[i].comments > 0 && typeof noisyData === 'undefined') {
      noisyData = data[i]
    } else if (data[i].comments > 0 && data[i].comments > noisyData.comments) {
      noisyData = data[i]
    }
  }
  if (countIssue > 1) {
